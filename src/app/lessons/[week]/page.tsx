...
const words = await db.word.findMany({ where: { week }, orderBy: { english: "asc" } });
...
{words.map(w => (
<li key={w.id} className="rounded border bg-white p-3 flex justify-between">
<div>
<div className="font-medium">{w.english}</div>
<div className="text-sm text-gray-600">{w.thai} · {w.roman}</div>
</div>
